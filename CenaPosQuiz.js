class CenaPosQuiz extends Phaser.Scene {
  constructor() {
    super({ key: "CenaPosQuiz" });

    this._linhas = [
      {
        personagem: "Vendedora",
        texto:
          "Vejo que você sabe bastante coisa! Porém, vamos aprender mais na fábrica!",
        avatar: "npcLoja",
      },
      {
        personagem: "Jogador",
        texto: "Na fábrica? Ótimo! Vou conhecer como os produtos são feitos!",
        avatar: "persona4",
      },
      {
        personagem: "Vendedora",
        texto:
          "Perfeito! Vamos lá. A fábrica fica no fim do corredor, à esquerda.",
        avatar: "npcLoja",
      },
    ];
  }
  ...
  _encerrar() {
    this.time.delayedCall(100, () => {
      ...
      this.scene.stop("CenaPosQuiz");
      // Chama o método da Cenaloja para mostrar a indicação
      this.scene.get("Cenaloja")._mostrarIndicacaoFabrica();
    });
  }
}